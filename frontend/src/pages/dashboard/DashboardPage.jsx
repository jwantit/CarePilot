        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-400 uppercase tracking-wider mb-1">성공률</div>
              <div
                className={`text-sm font-semibold ${
                  (callStats.changeRate || 0) === 0 ? "text-slate-400" : "text-teal-400"
                }`}
              >
                {loading
                  ? "로딩 중..."
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-400 uppercase tracking-wider">
                <span className="text-yellow-400 font-semibold">
                  {loading ? "로딩 중..." : `미처리 ${notificationStats.unprocessed || 0}건`}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-sm p-6 shadow-lg hover:shadow-xl transition-shadow min-h-[400px]">
          ...
          <div className="space-y-3">
            {urgentPatients.map((patient) => (
              <div key={patient.careTargetId} className="bg-slate-700/30 rounded-sm p-4 border border-slate-600">
            ...
            {waitingTasks
              ...
              .map((task) => (
                <div key={task.taskId} className="bg-slate-700/30 rounded-sm p-4 border border-slate-600">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-sm text-slate-200 truncate">
            ...
            {urgentNotifications.map((notification) => (
              <div key={notification.notificationId} className="bg-slate-700/30 rounded-sm p-4 border border-slate-600">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-sm text-slate-200 truncate">
          ...
          <div className="space-y-3 flex-1">
            {todaySchedules.map((schedule) => {
              ...
              const getScheduleStatus = () => {
                ...
                if (now >= scheduledDate && schedule.status !== 'COMPLETED') return '[진행 중..]';
                return '[예정]';
              };
              ...
              return (
                <div key={schedule.scheduleId} className="bg-slate-700/30 rounded-sm p-4 border border-slate-600">
                  <div className="flex items-center justify-between gap-4">
                    <span className={`text-sm ${isCompleted ? 'text-teal-400' : isInProgress ? 'text-slate-400' : 'text-slate-200'} truncate`}>

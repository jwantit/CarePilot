...
// ----- AITask -----
const AI_TASK_STATUS_LABEL = {
  WAITING: '대기',
  SUCCESS: '성공',
  FAILED: '실패',
};

const AI_TASK_STATUS_COLOR = {
  WAITING: 'bg-yellow-100 text-yellow-700 border-yellow-300',
  SUCCESS: 'bg-green-100 text-green-700 border-green-300',
  FAILED: 'bg-red-100 text-red-700 border-red-300',
};

const AI_TASK_TYPE_LABEL = {
  CALL_INIT: '전화 발신',
...
export const getAITaskStatusLabel = (status) => AI_TASK_STATUS_LABEL[status] ?? status;
export const getAITaskStatusColor = (status) => AI_TASK_STATUS_COLOR[status] ?? 'bg-gray-100 text-gray-700 border-gray-300';
export const getAITaskTypeLabel = (type) => AI_TASK_TYPE_LABEL[type] ?? type;
...
export const AI_TASK_STATUS_OPTIONS = [
  { value: '', label: '전체' },
  ...Object.entries(AI_TASK_STATUS_LABEL).map(([value, label]) => ({ value, label })),
];
export const AI_TASK_TYPE_OPTIONS = [
  { value: '', label: '전체' },
  ...Object.entries(AI_TASK_TYPE_LABEL).map(([value, label]) => ({ value, label })),
];

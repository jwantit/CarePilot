...
const CareGroupAddMemberModal = ({ isOpen, onClose, selectedIds, setSelectedIds, onSave, existingMemberIds = [] }) => {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-cp-card bg-gradient-to-br from-cp-card to-cp-bg border border-cp-border w-full max-w-lg rounded-sm shadow-xl overflow-hidden flex flex-col h-[80vh]">
        {/* 헤더 */}
        <div className="relative px-6 py-4 bg-cp-bg/30 border-b border-cp-border">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h2 className="text-xl font-bold text-cp-text tracking-tight leading-tight">대상자 추가</h2>
              <p className="text-xs font-semibold text-cp-muted mt-0.5">Select New Members Only</p>
            </div>
          </div>
        </div>

        {/* 검색창 */}
        <div className="px-6 py-4 bg-cp-bg/30 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto modal-scrollbar px-2">
          {loading ? (
            <div className="h-full flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs font-semibold text-cp-muted">로딩 중...</p>
            </div>
                  ...
                  >
                    <div className="flex-1 flex flex-col text-left">
                        <div className="flex items-center gap-2">
                          <span className={`text-sm font-bold ${isSelected ? 'text-teal-400' : 'text-cp-text'}`}>
                            {p.name}
                          </span>
                        ...
                        <div className="flex items-center text-xs font-medium text-cp-muted gap-1.5 mt-0.5">
            ...
            <div className="h-full flex flex-col items-center justify-center text-cp-muted">
              <div className="w-20 h-20 bg-cp-bg border-2 border-cp-border rounded flex items-center justify-center mb-5">
                <Search size={32} className="text-cp-muted/40"/>
              </div>
              <p className="font-semibold text-sm text-cp-text mb-2">// No patients available</p>
              <p className="text-xs text-cp-muted font-mono">추가 가능한 환자가 없습니다.</p>
            </div>
          )}
        </div>

        {/* 푸터 */}
        <div className="p-6 border-t border-cp-border bg-cp-bg/30 shrink-0">

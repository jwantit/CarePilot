      ...
      <div className="max-h-[500px] overflow-y-auto modal-scrollbar">
        <table className="w-full text-center table-fixed border-separate border-spacing-0">
          <thead className="bg-cp-header sticky top-0 z-10 border-b-2 border-teal-500/30">
            <tr>
              <th className="px-10 py-4 w-[12%] text-sm font-semibold text-white dark:text-teal-400 uppercase tracking-widest">처방일</th>
              <th className="px-6 py-4 w-[12%] text-sm font-semibold text-white dark:text-teal-400 uppercase tracking-widest">분석 일시</th>
              <th className="px-6 py-4 w-[46%] text-sm font-semibold text-white dark:text-teal-400 uppercase tracking-widest">요약 및 분석</th>
              <th className="px-10 py-4 w-[15%] text-sm font-semibold text-white dark:text-teal-400 uppercase tracking-widest text-center">처방전 이미지</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-cp-border">
            {list && list.length > 0 ? (
              list.map((row) => {
                ...
                return (
                  <tr key={row.prescriptionId} className="hover:bg-cp-bg/50 transition-all group bg-cp-card/30">
                    <td className="px-10 py-5 text-base text-cp-text tabular-nums align-top">
                      {row.prescribedDate || '-'}
                    </td>
                    <td className="px-6 py-5 text-base font-normal text-cp-muted tabular-nums align-top">
                      {row.analyzedAt || '-'}
                    </td>
                    <td className="px-6 py-5 align-top">
                      <div className="text-base text-cp-muted font-normal group-hover:text-cp-text whitespace-pre-wrap text-left">
                        {row.summary ? (
                          <>
                    ...
                    <td className="px-10 py-5 text-center align-top">
                      {row.imageFileUrl ? (
                        <a
                          ...
                          className="cp-link-blue text-base font-medium"
                        >
                          이미지 보기
                        </a>
                      ) : (
                        <span className="text-cp-muted text-base">-</span>
                      )}
                    </td>
                  </tr>
                );

...
public interface CallScheduleRepository extends JpaRepository<CallSchedule, Long> {
    ...
    // 조직별 특정 월의 스케줄 조회 (캘린더용)
    List<CallSchedule> findByOrganizationOrganizationIdAndScheduledTimeBetween(Long organizationId, LocalDateTime start, LocalDateTime end);

    // 예약된 통화 목록 조회 (여러 상태 허용)
    List<CallSchedule> findByStatusInOrderByScheduledTimeAsc(Collection<ScheduleStatus> statuses);

    // 조직별 예약된 통화 목록 조회 (여러 상태 허용)
    List<CallSchedule> findByOrganizationOrganizationIdAndStatusInOrderByScheduledTimeAsc(Long organizationId, Collection<ScheduleStatus> statuses);
